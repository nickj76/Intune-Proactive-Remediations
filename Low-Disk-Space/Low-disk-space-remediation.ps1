...
param(
    ...
    [string]$Config = "https://rawcdn.githack.com/nickj76/Scripts/a7594e3f7c490aba938af286a19067d940f76743/New-Toast-Note/xml/config-toast-lowdiskspace.xml"
)
...
if (($UpgradeOS -eq "True") -AND ($PendingRebootCheck -eq "True")) {
    ...
    Write-Log -Level Error -Message "Error. You can't have both ÃpgradeOS feature set to True AND PendingRebootCheck feature set to True at the same time. Check your config"
    Exit 1
}
if (($UpgradeOS -eq "True") -AND ($PendingRebootUptime -eq "True")) {
    Write-Log -Level Error -Message "Error. Conflicting selection in the $Config file" 
    Write-Log -Level Error -Message "Error. You can't have both ÃpgradeOS feature set to True AND PendingRebootUptime feature set to True at the same time. Check your config"
    Exit 1
}
